...
required_files = [
    ...
    ROOT / "OsterConflict" / "PASS45_BATCH_RUNTIME.cmd",
]
...
if launcher_path.is_file():
    ...
    # START_HERE remains the only user-facing entry point. The current full-test route is the
    # canonical Pass45 batch runtime owner. Historical R14 strict wrappers are not allowed to
    # force the launcher back through retired orchestration merely to satisfy this location verifier.
    ...
    if 'call "%~dp0OsterConflict\\PASS45_BATCH_RUNTIME.cmd"' not in launcher:
        errors.append("START_HERE.cmd does not route full runtime test to the current Pass45 batch owner")
    if 'call "%~dp0RUN_R14_MAIN_RUNTIME_ACCEPTANCE.cmd"' in launcher:
        errors.append("START_HERE.cmd regressed to the retired R14 strict acceptance wrapper")
    if 'call "%~dp0RUN_R14_PLAYFLOW_PERFORMANCE_ACCEPTANCE.cmd"' in launcher:
        errors.append("START_HERE.cmd bypasses the current Pass45 batch owner by calling playflow directly")
...
print("START_HERE.cmd is the single user entry point; full runtime enters the current Pass45 batch owner.")
