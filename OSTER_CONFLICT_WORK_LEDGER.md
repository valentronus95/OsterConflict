...
- Pass45 corrective PR #83: **MERGED**.
- PR #83 source head: `f89841ca9375ed5b8da496ec36e8c2efe2a8a437`.
- PR #83 merge on `main`: `f5e883fb69ae8bdd35c754dc895d8b06e4843e08`.
- PR #83 current-head CI: **GREEN** across aggregate Source verification and the relevant Pass15–45 / production / Museum / launcher contracts.
...
- Current status token: **PASS 45 ACTIVE / PR #83 MERGED + SOURCE CI GREEN / POST-MERGE CONTENT CLOSURE CODED_UNTESTED / LATEST LOCAL UE RUNTIME REJECTED 2026-08-25**.
...
Pass45 remains **RUNTIME REJECTED** until a newer factual local run supersedes this evidence. Latest rejected runtime showed:
...
- `VERIFY_PASS45_STALE_RUNTIME_RETIREMENT.py` blocks resurrection of that generic decorator family;
- `VERIFY_OSTER_WORLD_MODELS_PASS.py` forward-ported to current truthful ownership instead of requiring the rejected family.

Current PR #85 head source CI after the world-model forward-port: **GREEN** for all completed current-head workflows including aggregate `Source verification`, Pass45 retirement, Pass15/16/17/18/19, Pass38–44, production vehicle/rack and Oster world-model contracts.

Important limitation: one separate `OCEnterableHouse` yard owner still contains `RealYardFence` / `RealSideShed`; it is now explicitly identified for the next safe source patch. Do not claim the dark tower/shack is fully runtime-closed until this remaining owner is retired/referenced and a local screenshot confirms the artifact is gone.

## 6. Active requirements

| ID | Requirement | Status | Current action |
|---|---|---|---|
| STALE-OWNER-001 | Old code/verifiers must not resurrect rejected behavior | CODED_UNTESTED | Retirement gates active and green. |
...
| LOC-TOWER-001 | Remove unreferenced dark tower/shack | IN_PROGRESS / CODED_UNTESTED | Mass decorator Side_Shed retired; separate EnterableHouse Side_Shed remains to close safely. |
| VIS-GENERIC-RESIDENTIAL-001 | No rejected generic village house/fence replacement | CODED_UNTESTED | Mass decorator family retired; semantic baseline restored. |
...
| VEH-BTR-001 | BTR-4 proportions/orientation correct | CODED_UNTESTED / CONTENT CHECK | Uniform scale + axis correction; white material artifact still open. |
...
| WEAPON-MATERIAL-001 | Rack weapons have authored materials/textures | CODED_UNTESTED / CONTENT CHECK | Missing/default slots fail visibly. |
...
14. decorator `Side_Shed` replacement or hiding of `ResidentialRoofs` / `ResidentialDetails`.
...
10. [x] PR #83 current-head source CI fully green.
11. [x] Merge PR #83 to `main` at `f5e883fb69ae8bdd35c754dc895d8b06e4843e08`.
12. [x] Trace mass generic residential house/fence/Side_Shed owner to `AOCAssetModelDecorator`.
13. [x] Retire that mass generic residential replacement and restore semantic roof/detail baseline.
14. [x] Forward-port world-model verifier; PR #85 current-head CI green for the current head checked here.
15. [ ] Safely retire/reference the remaining `OCEnterableHouse` exterior `RealYardFence` / `RealSideShed` path.
16. [ ] Add the generic-residential retirement marker to cumulative runtime acceptance.
17. [ ] Keep BTR white/default material and weapon material/texture gaps fail-visible; close only with factual asset evidence.
18. [ ] Refresh PR #85 after final source/docs scope and obtain fresh final-head CI.
19. [ ] Merge PR #85 only after final-head CI is green.
20. [ ] Run local `START_HERE.cmd -> 1. ЗВИЧАЙНА ГРА` factual UE 5.8 acceptance.
