...
5. Tactical-map component auto-fit / historical 800 m half-width minimum that expands `M` beyond the user-approved compact area.
6. Runtime `BasicShapeMaterial` material recovery that paints missing weapon materials grey/dark and then calls the visual repaired.
7. Pass 37/38 palette mutation that creates runtime MIDs or calls `SetMaterial()` on placeholder weapon slots.
8. All-or-nothing vehicle import where missing BTR prevents available HMMWV/M2 from importing.
9. Launcher text that prints all production vehicles imported after the importer actually failed.
...
- runtime acceptance requires primary compact-world and compact-tactical-map evidence in addition to the existing Pass 44 gates;
- Pass 3/4/19/20/33/36/37/38/41 and launcher/tactical-map stale verifier expectations forward-ported/retired where they conflicted.
...
4. Confirm `PASS44_COMPACT_PLAYABLE_AREA_READY`.
5. Open `M` and confirm `PASS44_TACTICAL_MAP_COMPACT_BOUNDS_READY`; visually verify the map no longer expands beyond the compact central-Oster reference.
6. Confirm `PASS44_ACTUAL_PAWN_MUSEUM_BASE_READY` and visually verify Museum is actually nearby.
7. Check rack authored materials; any `PASS44_WEAPON_RACK_AUTHORED_MATERIAL_GAP` keeps visual acceptance FAIL.
8. Remain in gameplay ~20 s only while thermals are sane; require >=30 FPS and no progressive collapse.
9. Verify HMMWV/M2/BTR individually. A missing one is a named content gap, not a hidden proxy success.
