...
- Active correction branch: `fix/runtime-acceptance-pass-4` → `main`
...
1. Build/CI pass 4; source verifier та focused runtime contracts мають пройти до merge.
2. Runtime acceptance: deployment 0–100 без freeze, Museum spawn/rack, minimap, rear gunner, reverse steering, grass, tracer саме з дула.
3. Під час `START_HERE.cmd → 1` normal launcher має зупинятись ДО гри, якщо required weapon visuals або HMMWV/M2/BTR не відкриваються у fresh UE process.
4. Exact M249/Remington production identity лишається content gap; real R13 fallback допускається лише як позначений fallback, не як exact production asset.
5. Global landmark separation acceptance at canonical Museum/Silpo/Culture anchors.
6. Separate detail branches: Museum, Stadium, Silpo, Culture House; merge without moving canonical geo anchors.
7. Distant flicker/duplicate geometry and roads/sidewalk geometry pass.
8. Real Oster house variation and large-building/stairs detail pass.
