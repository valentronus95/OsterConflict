...
def symmetric_inside_interval(center: tuple[float, float], length: float, width: float, yaw: float) -> tuple[float, float, float]:
    # AddRoadWithWalks places each 260 cm sidewalk at half-road-width + 260 cm from the centerline.
    # Therefore the complete road+two-walk envelope reaches another 130 cm beyond each sidewalk center.
    envelope = width * 0.5 + 260.0 + 130.0
    angle = math.radians(yaw)
    ux, uy = math.cos(angle), math.sin(angle)
    vx, vy = -uy, ux
    x_interval = axis_interval(
        center[0], ux,
        xmin + envelope * abs(vx),
        xmax - envelope * abs(vx),
    )
    y_interval = axis_interval(
        center[1], uy,
        ymin + envelope * abs(vy),
        ymax - envelope * abs(vy),
    )
    half = length * 0.5
...
if len(road_records) != 14:
    fail(f"expected 14 explicit road records after symmetric splits, parsed {len(road_records)}")
...
split_specs = [
    (
        "KrushelnytskaSpineSegments",
        ["S01_KR_SPINE_SOUTH_SHARED", "S01_KR_SPINE_INSIDE", "S01_KR_SPINE_NORTH_SHARED"],
        (-33500.0, 25000.0), 112000.0, 920.0, 91.5,
        ["Crossing", "Inside", "Crossing"],
    ),
    (
        "EastWest02Segments",
        ["S01_EW02_INSIDE", "S01_EW02_EAST_SHARED"],
        (-18000.0, 17000.0), 61000.0, 820.0, 0.0,
        ["Inside", "Crossing"],
    ),
    (
        "WorldDiag01Segments",
        ["S01_DIAG01_INSIDE", "S01_DIAG01_EAST_SHARED"],
        (-23500.0, 40500.0), 51000.0, 760.0, 18.0,
        ["Inside", "Crossing"],
    ),
    (
        "WorldDiag02Segments",
        ["S01_DIAG02_INSIDE", "S01_DIAG02_EAST_SHARED"],
        (-5000.0, 33500.0), 49000.0, 760.0, -34.0,
        ["Inside", "Crossing"],
    ),
    (
        "ParkSouthSegments",
        ["S01_PARK_SOUTH_WEST_SHARED", "S01_PARK_SOUTH_INSIDE"],
        (park[0], park[1] - 8500.0), 43000.0, 720.0, 2.0,
        ["Crossing", "Inside"],
    ),
]
expected_split_ids = {rid for _name, ids, *_rest in split_specs for rid in ids}
if set(records_by_id) != expected_unsplit_ids | expected_split_ids:
    fail(f"audited road ID set changed: {sorted(records_by_id)}")

for obsolete in [
    "S01_CROSS_KRUSHELNYTSKA_SPINE", "S01_CROSS_WORLD_EW_02",
    "S01_CROSS_WORLD_DIAG_01", "S01_CROSS_WORLD_DIAG_02", "S01_CROSS_PARK_SOUTH",
]:
    if obsolete in road_cpp:
        fail(f"obsolete unsplit audit record returned: {obsolete}")

# College approach remains fully inside; the two one-sided corridors intentionally remain unsplit/Crossing.
for rid in expected_unsplit_ids:
    record = records_by_id[rid]
    _rid, anchor, sx0, sy0, _sz0, ssx, ssy, _ssz, syaw, two_walks, declared = record
    cx, cy = resolve_center(anchor, float(sx0), float(sy0))
    actual = classify_oriented_rect(cx, cy, float(ssx), float(ssy), float(syaw))
    expected = "Inside" if rid == "S01_ROAD_COLLEGE_APPROACH" else "Crossing"
    if declared != expected or actual != expected:
        fail(f"unsplit corridor relation drift for {rid}: declared={declared}, road={actual}, expected={expected}")
    if rid != "S01_ROAD_COLLEGE_APPROACH" and two_walks != "false":
        fail(f"remaining unsplit corridor {rid} is no longer one-sided; symmetric split audit should be rerun")


def verify_symmetric_split(
    accessor: str,
    ids: list[str],
    ...
    if len(ids) != len(expected_relations):
        fail(f"internal verifier spec mismatch for {accessor}")

    angle = math.radians(original_yaw)
    ux, uy = math.cos(angle), math.sin(angle)
    vx, vy = -uy, ux
    half = original_length * 0.5
    safe_lo, safe_hi, envelope = symmetric_inside_interval(
        original_center, original_length, original_width, original_yaw
    )
    ...
    for rid, expected_relation in zip(ids, expected_relations):
        if rid not in records_by_id:
            fail(f"missing split record {rid}")
        record = records_by_id[rid]
        _rid, anchor, sx0, sy0, _sz0, ssx, ssy, ssz, syaw, two_walks, declared = record
        if anchor != "Absolute":
            fail(f"split segment {rid} must stay absolute to preserve its original world contour")
        ...
        if two_walks != "true" or declared != expected_relation:
            fail(f"sidewalk/ownership drift for {rid}: walks={two_walks}, relation={declared}")

        dx, dy = cx - original_center[0], cy - original_center[1]
        center_t = dx * ux + dy * uy
        perpendicular = dx * vx + dy * vy
        if abs(perpendicular) > TOL_CM:
            fail(f"split segment {rid} left the original centerline by {perpendicular:.6f} cm")
        start_t = center_t - length * 0.5
        end_t = center_t + length * 0.5
        intervals.append((rid, start_t, end_t))
        total_length += length

        envelope_relation = classify_oriented_rect(cx, cy, length, envelope * 2.0, yaw)
        if envelope_relation != expected_relation:
            fail(f"full sidewalk-envelope relation drift for {rid}: {envelope_relation} != {expected_relation}")
    ...
    for previous, current in zip(intervals, intervals[1:]):
        ...
        if abs(gap) > CONTINUITY_TOL_CM:
            fail(f"{accessor} gap/overlap between {previous[0]} and {current[0]}: {gap:.6f} cm")

    if len(inside_intervals) != 1:
        fail(f"{accessor} must have exactly one S01-owned inside segment")
    actual_lo, actual_hi = inside_intervals[0]
    # Stored cuts may be nudged up to 0.001 cm inward to keep finite-decimal records strictly inside.
    if abs(actual_lo - safe_lo) > 0.002 or abs(actual_hi - safe_hi) > 0.002:
        fail(
            f"{accessor} ownership cut drift: actual [{actual_lo:.6f},{actual_hi:.6f}], "
            f"safe [{safe_lo:.6f},{safe_hi:.6f}]"
        )
...
expected_path_ids = {
    "S01_PATH_PARK_EW", "S01_PATH_PARK_NS", "S01_PATH_PARK_DIAG_E", "S01_PATH_PARK_DIAG_W",
...
for record in path_records:
    ...
    cx, cy = resolve_center(anchor, float(sx0), float(sy0))
    if classify_oriented_rect(cx, cy, float(ssx), float(ssy), float(syaw)) != "Inside":
        fail(f"owned path no longer fits wholly inside S01: {pid}")

# Park -> CultureParkNorth derived path remains Crossing/shared.
...
# Runtime ownership contracts and removal of all already-split direct calls.
for token in [
...
if "FOCLocationSectorS01RoadData::SharedCrossingCorridors()" in world:
    fail("unsplit one-sided audit records must not be rendered from the registry")
...
# Only the two one-sided crossing roads and the derived park path remain direct/shared.
for token in [
    ...
    if token not in world:
        fail(f"one-sided/shared geometry changed before asymmetric split audit: {token}")

print("R13 LOCATION-FIRST S01 ROAD TOPOLOGY VERIFY: PASS")
print(
    f"S01 bounds approx X[{xmin:.1f},{xmax:.1f}] Y[{ymin:.1f},{ymax:.1f}] cm; "
    "5 symmetric crossing corridors are continuity-preserving explicit splits; "
    "only 2 one-sided road crossings plus Park->NorthCivic remain unsplit/shared."
)
