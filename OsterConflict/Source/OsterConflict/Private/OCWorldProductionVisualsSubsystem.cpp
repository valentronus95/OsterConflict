...
namespace
{
    ...
    constexpr const TCHAR* RoadMaterialPath = TEXT("/Game/TileableForestRoad/Materials/SM_Forest_Path_Base_Mat.SM_Forest_Path_Base_Mat");
    ...
    FVector SourceBoxWorldSize(const FTransform& Transform)
    {
        ...
        // OCWorldSectorOster::AddBox encodes centimetre dimensions as Scale = SizeCm / 100 on Engine Cube.
        return Scale * 100.0f;
    }
        ...
        const FVector& DesiredBoxCm)
    {
        ...
        // Preserve the authored model's proportions. The hidden cube remains collision authority, so the visual
        // does not need to be stretched independently on all axes to impersonate the old box.
        const float FitX = DesiredBoxCm.X / MeshSize.X;
        const float FitY = DesiredBoxCm.Y / MeshSize.Y;
        const float UniformScale = FMath::Clamp(FMath::Min(FitX, FitY), 0.15f, 12.0f);
        ...
        UStaticMesh* MeshB)
    {
        ...
        for (int32 Index = 0; Index < Source->GetInstanceCount(); ++Index)
        {
            ...
            const FVector DesiredSize = SourceBoxWorldSize(SourceTransform);
        ...
        UStaticMesh* FenceMesh)
    {
        ...
        for (int32 Index = 0; Index < Source->GetInstanceCount(); ++Index)
        {
            ...
            const float NominalSegmentLength = FMath::Max(100.0f, MeshLength * HeightScale);
            const int32 Count = FMath::Clamp(FMath::CeilToInt(TargetLength / NominalSegmentLength), 1, 64);
            const float SegmentLength = TargetLength / static_cast<float>(Count);
            const float LengthScale = SegmentLength / MeshLength;

            FRotator SourceRotation = SourceTransform.Rotator();
            if (!bMeshLengthX) SourceRotation.Yaw += 90.0f;
            const FVector Direction = SourceRotation.RotateVector(FVector::ForwardVector);
            const float FirstOffset = -TargetLength * 0.5f + SegmentLength * 0.5f;

            for (int32 Segment = 0; Segment < Count; ++Segment)
            {
                const float Along = FirstOffset + Segment * SegmentLength;
                FVector Location = SourceTransform.GetLocation() + Direction * Along;
                ...
                FTransform VisualTransform(SourceRotation, Location, Scale);
                Target->AddInstance(VisualTransform, true);
                ++Added;
            }
        }
...
{
    if (UWorld* World = RuntimeWorld.Get())
    {
...
void UOCWorldProductionVisualsSubsystem::TryBuildProductionVisuals()
{
    ...
    if (!HouseA || !HouseB || !FenceA || !FenceB || !FenceC || !FenceD || !GroundMaterial || !RoadMaterial)
    {
        UE_LOG(LogTemp, Error,
            TEXT("PASS45_B2_PRODUCTION_VISUALS_FAIL reason=required_imported_asset_missing houseA=%d houseB=%d fenceA=%d fenceB=%d fenceC=%d fenceD=%d groundMat=%d roadMat=%d"),
            HouseA ? 1 : 0, HouseB ? 1 : 0, FenceA ? 1 : 0, FenceB ? 1 : 0,
            FenceC ? 1 : 0, FenceD ? 1 : 0, GroundMaterial ? 1 : 0, RoadMaterial ? 1 : 0);
        return;
    }
    ...
    Owner->SetActorLabel(TEXT("Pass45_ProductionEnvironmentVisuals"));
    ...
    if (Buildings && HouseInstances == Buildings->GetInstanceCount() && HouseInstances > 0)
    {
    ...
    else
    {
        UE_LOG(LogTemp, Warning,
            ...
            Buildings ? Buildings->GetInstanceCount() : -1, HouseInstances);
    }
    ...
    if (Sidewalks) Sidewalks->SetMaterial(0, RoadMaterial);

    bBuilt = true;
    World->GetTimerManager().ClearTimer(RetryHandle);

    UE_LOG(LogTemp, Display,
        TEXT("PASS45_B2_PRODUCTION_VISUALS_READY houses=%d fence_segments=%d real_house_meshes=2 real_fence_meshes=4 ground_material=real road_material=real polling_after_ready=0 collision_backstop_retained=1"),
        HouseInstances, FenceInstances);
    UE_LOG(LogTemp, Display,
        TEXT("PASS45_B2_REMAINING_CONTENT_GAPS landmark_generic_blocks=1 park_detail_proxies=1 dedicated_site_owners_override_museum_silpo_culture=1"));
}
