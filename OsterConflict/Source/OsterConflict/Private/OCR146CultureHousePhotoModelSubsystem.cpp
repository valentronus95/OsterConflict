...
void UOCR146CultureHousePhotoModelSubsystem::BuildCultureHouse(UWorld& World) const
{
    ...
    UStaticMesh* Wall8 = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Wall_8m.Wall_8m")).ResolveObject());
    UStaticMesh* WindowWall4 = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Wall_Window_4m.Wall_Window_4m")).ResolveObject());
    UStaticMesh* DoorWindowWall8 = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Wall_Door_Windows_8m.Wall_Door_Windows_8m")).ResolveObject());
    UStaticMesh* WallTop4 = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Wall_Top_4m.Wall_Top_4m")).ResolveObject());
    UStaticMesh* Pillar = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Wall_Pillar.Wall_Pillar")).ResolveObject());
    UStaticMesh* Door = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Door_01.Door_01")).ResolveObject());
    UStaticMesh* RoofTile = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Porch_Roof_8x4m.Porch_Roof_8x4m")).ResolveObject());
    UStaticMesh* BottomExtender = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Bottom_Extender_4m.Bottom_Extender_4m")).ResolveObject());
    UStaticMesh* Porch = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Porch_4x4m.Porch_4x4m")).ResolveObject());
    UStaticMesh* WindowFramePart = Cast<UStaticMesh>(FSoftObjectPath(
        TEXT("/Game/Modular_Rural_Cabin/Meshes/Modular/Window_Frame_Part.Window_Frame_Part")).ResolveObject());

    if (!Wall8 || !WindowWall4 || !DoorWindowWall8 || !WallTop4 || !Pillar || !Door ||
        !RoofTile || !BottomExtender || !Porch || !WindowFramePart)
    {
        UE_LOG(LogTemp, Error,
            TEXT("PASS45_CULTURE_HOUSE_AUTHORED_SHELL_FAIL reason=preload_gap modular_root=%s sync_load=0 basicshape_fallback=0 runtime_acceptance=0"),
            ModularRoot);
        return;
    }
    ...
    UInstancedStaticMeshComponent* FrontShell = MakeAuthoredISM(Model, Root, DoorWindowWall8, TEXT("R146Culture_AuthoredFrontShell"), true);
    UInstancedStaticMeshComponent* WindowShell = MakeAuthoredISM(Model, Root, WindowWall4, TEXT("R146Culture_AuthoredWindowShell"), true);
    UInstancedStaticMeshComponent* RearShell = MakeAuthoredISM(Model, Root, Wall8, TEXT("R146Culture_AuthoredRearShell"), true);
    UInstancedStaticMeshComponent* TopTrim = MakeAuthoredISM(Model, Root, WallTop4, TEXT("R146Culture_AuthoredTopTrim"), false);
    UInstancedStaticMeshComponent* Columns = MakeAuthoredISM(Model, Root, Pillar, TEXT("R146Culture_Columns"), false);
    UInstancedStaticMeshComponent* Doors = MakeAuthoredISM(Model, Root, Door, TEXT("R146Culture_AuthoredDoors"), false);
    UInstancedStaticMeshComponent* Roof = MakeAuthoredISM(Model, Root, RoofTile, TEXT("R146Culture_AuthoredRoof"), false);
    UInstancedStaticMeshComponent* Foundation = MakeAuthoredISM(Model, Root, BottomExtender, TEXT("R146Culture_AuthoredFoundation"), true);
    UInstancedStaticMeshComponent* Ground = MakeAuthoredISM(Model, Root, Porch, TEXT("R146Culture_AuthoredForecourt"), true, false);
    UInstancedStaticMeshComponent* Frames = MakeAuthoredISM(Model, Root, WindowFramePart, TEXT("R146Culture_AuthoredWindowFrames"), false);
    ...
    for (const float X : FacadeXs)
    {
        ...
        AddFittedAuthoredMesh(RearShell, FVector(X, 925.0f, 365.0f), FVector(800.0f, 52.0f, 730.0f), FRotator(0.0f, 180.0f, 0.0f));
    ...
    {
        AddFittedAuthoredMesh(WindowShell, FVector(-1600.0f, Y, 365.0f), FVector(600.0f, 52.0f, 730.0f), FRotator(0.0f, -90.0f, 0.0f));
        AddFittedAuthoredMesh(WindowShell, FVector(1600.0f, Y, 365.0f), FVector(600.0f, 52.0f, 730.0f), FRotator(0.0f, 90.0f, 0.0f));
    }
    ...
    for (const float X : FacadeXs)
    {
        ...
        AddFittedAuthoredMesh(Foundation, FVector(X, 925.0f, 42.0f), FVector(800.0f, 100.0f, 84.0f), FRotator(0.0f, 180.0f, 0.0f));
    }
    for (const float Y : SideYs)
    {
        AddFittedAuthoredMesh(Foundation, FVector(-1600.0f, Y, 42.0f), FVector(600.0f, 100.0f, 84.0f), FRotator(0.0f, -90.0f, 0.0f));
        AddFittedAuthoredMesh(Foundation, FVector(1600.0f, Y, 42.0f), FVector(600.0f, 100.0f, 84.0f), FRotator(0.0f, 90.0f, 0.0f));
    }
    ...
    UE_LOG(LogTemp, Display,
        TEXT("PASS45_CULTURE_HOUSE_AUTHORED_SHELL_READY site=Hranovskoho3 x=%.1f y=%.1f authored_wall=1 authored_pillar=1 authored_door=1 authored_roof=1 authored_forecourt=1 basicshape_visible=0 basicshape_material=0 six_column_facade=1 legacy_owner_mutation=0 sync_load=0 prerequisite_resident=1 runtime_visual_acceptance=pending"),
        Site.X, Site.Y);
    UE_LOG(LogTemp, Display,
