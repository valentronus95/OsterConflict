...
namespace
{
    ...
    bool IsLegacyBaseOwnedComponent(const UInstancedStaticMeshComponent* Component)
    {
        ...
        {
            TEXT("Sidewalks"), TEXT("Roads"), TEXT("Buildings"),
            TEXT("ResidentialRoofs"), TEXT("ParkDetails"), TEXT("MetalFences")
        };
...
void UOCRuntimeAcceptancePass6Subsystem::OnWorldBeginPlay(UWorld& InWorld)
{
    ...
    // The frontend shell uses the same map package. Do not mutate world-sector instances or weapon presentation
    // before the actual gameplay world exists.
...
void UOCRuntimeAcceptancePass6Subsystem::RemoveLegacyGameplayBaseInstances()
{
    ...
    UE_LOG(LogTemp, Display,
        TEXT("Pass 8 legacy BASE cleanup complete: removed %d obsolete generated instance(s)."),
        RemovedInstances);
}
...
void UOCRuntimeAcceptancePass6Subsystem::NormalizeProductionStaticWeapons()
{
    ...
    for (TActorIterator<AOCWeaponBase> It(World); It; ++It)
    {
        ...
        for (UStaticMeshComponent* Component : Components)
        {
            ...
            UE_LOG(LogTemp, Display,
                TEXT("Pass 8 normalized production StaticMesh weapon '%s' (%s) to X-forward; imported materials preserved."),
