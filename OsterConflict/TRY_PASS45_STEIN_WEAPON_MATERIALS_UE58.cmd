...
if exist "%SENTINEL%" (
  ...
  findstr /L /C:"STATUS=EDITOR_IMPORT_VALIDATED_RUNTIME_VISUAL_PENDING" "%SENTINEL%" >nul && (
    echo [WEAPONS] Stein authored material import already matches current Pass45 UE58 explicit-binding revision.
    exit /b 0
  )
)
...
echo [WEAPONS] Stein material cache is missing or stale. Running texture-first + explicit UE58 authored binding...
call "%REIMPORT%"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
