...
# -----------------------------------------------------------------------------
# 1. First-person arms / AK animation donor. The bundled AK animation subtree
#    carries its own CC BY 4.0 attribution and must retain its license file.
# -----------------------------------------------------------------------------
$armsCommit = "c80a452e680c5a27c9e936176eb17e446869529a"
$armsArchive = Join-Path $CacheRoot "FPS-Arms-3D-$armsCommit.zip"
Get-RemoteFile -Id "OC-INTAKE-ARMS-001" `
    -Url "https://github.com/Ayush-Mohanty/FPS-Arms-3D/archive/$armsCommit.zip" `
    -Destination $armsArchive
...
# -----------------------------------------------------------------------------
# 2. Large CC0-source-asserted FPS kit. Only sfx + weapons are copied into the
#    Oster source tree. Bulk textures/HDRIs/scripts stay in the local cache.
# -----------------------------------------------------------------------------
if (-not $SkipLargeFpsKit) {
    ...
    Get-RemoteFile -Id "OC-INTAKE-FPSKIT-001" `
        -Url "https://github.com/petroulacl/fps-asset-kit/archive/$kitCommit.zip" `
        -Destination $kitArchive
...
# -----------------------------------------------------------------------------
# 3. Individually pinned CC0 candidate recordings.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 4. Direct upstream ambience assets with independently recorded SHA-256 values.
# -----------------------------------------------------------------------------
Get-RemoteFile -Id "OC-INTAKE-AMBIENCE-001" `
    -Url "https://opengameart.org/sites/default/files/Forest_Ambience.mp3" `
    -Destination (Join-Path $IntakeRoot "OpenGameArt\Ambience\Forest_Ambience.mp3") `
    -ExpectedSha256 "9850AA1D0D5D66BD9C5DAF8BB77C6D852E01F2F4DE22F283BD5621E8BED13B75"

Get-RemoteFile -Id "OC-INTAKE-FIRE-001" `
    -Url "https://opengameart.org/sites/default/files/fire.wav" `
    -Destination (Join-Path $IntakeRoot "OpenGameArt\Ambience\fire.wav") `
    -ExpectedSha256 "85CA0CC60D0C037FFF8B185E31AD1FCDBDA6CE45EEE17C3EE1318D1B8F59E330"

# Preserve source license/provenance files beside the acquired source trees.
Get-RemoteFile -Id "FPS-Arms-3D nested AK license" `
    -Url "https://raw.githubusercontent.com/Ayush-Mohanty/FPS-Arms-3D/$armsCommit/Models/fps_ak-74m_animations/license.txt" `
    -Destination (Join-Path $IntakeRoot "FPSArms3D\AK74M_LICENSE_CC_BY_4.0.txt")
Get-RemoteFile -Id "FPS-Arms-3D repository license" `
    -Url "https://raw.githubusercontent.com/Ayush-Mohanty/FPS-Arms-3D/$armsCommit/LICENSE" `
    -Destination (Join-Path $IntakeRoot "FPSArms3D\REPOSITORY_LICENSE.txt")
Get-RemoteFile -Id "hard-lines audio provenance" `
    -Url "https://raw.githubusercontent.com/yegors/hard-lines/$hardLinesCommit/public/audio/README.md" `
    -Destination (Join-Path $IntakeRoot "HardLines\AUDIO_PROVENANCE.md")
Get-RemoteFile -Id "border-run audio provenance" `
    -Url "https://raw.githubusercontent.com/yashimosh/border-run/$borderCommit/CREDITS.md" `
    -Destination (Join-Path $IntakeRoot "BorderRun\CREDITS.md")
Get-RemoteFile -Id "fps-asset-kit source README" `
