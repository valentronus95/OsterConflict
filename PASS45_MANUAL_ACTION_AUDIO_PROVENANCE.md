...
- Current repository-owned BoltCycle runtime asset: **CONTENT GAP**; a repository-owned CC0 source derivative is now tracked through Git LFS and awaits UE SoundWave import/wiring.
- Current repository-owned LeverCycle runtime asset: **CONTENT GAP**; a repository-owned CC0 source derivative is now tracked through Git LFS and awaits UE SoundWave import/wiring.
...
- Intended role: **real lever-action mechanical donor** for `LeverCycle` after the payload is imported and accepted in UE 5.8.
...
- Intended role: **real bolt-action mechanical donor** for `BoltCycle` after an accepted deterministic edit/import and UE 5.8 acceptance.
...
6. `BoltCycle` and `LeverCycle` remain unassigned/fail-visible until accepted UE SoundWave assets exist in the canonical branch and UE 5.8 can load them.
...
Because Freesound requires account login for the original WAV download, repository intake does not fake an original-file acquisition. `PASS45_MANUAL_ACTION_AUDIO_INTAKE.py` uses an exact audited public Freesound LQ preview URL as the transport for each donor.
...
- repository acquisition still leaves `runtime_ready=0` / `ue_import_pending=1` until an actual UE SoundWave is imported and accepted;
...
## Current transport re-audit and repository acquisition — 2026-09-01
...
- deterministic derivative: `SOURCE_ASSETS/PASS45/ManualActionAudio/lever_action_cc0_preview_donor.wav`
- derivative SHA-256 / Git LFS OID: `417ba38e5e87b53ef3711784f821f1b3fc303ac8d4df19d9eda80fb776881542`
- derivative size: `92078` bytes
...
- deterministic derivative: `SOURCE_ASSETS/PASS45/ManualActionAudio/bolt_action_cc0_preview_donor.wav`
- derivative SHA-256 / Git LFS OID: `5e64820d532c11e91af3eedf96ab34a38df7b3dd066b0b1c9d67b3fe3f34c8a7`
- derivative size: `624078` bytes
- derivative duration: `6.500000 s`

Controlled write run `33501795799` completed **SUCCESS** and bot commit `b8b54ac019a559430fca8be7a1db4ef1f49d002f` added both Git LFS pointers plus `SOURCE_ASSETS/PASS45/ManualActionAudio/MANIFEST.json`. The workflow verified that each pointer OID/size exactly matches the manifest derivative identity.
...
- Import the two repository-owned derivatives to repository-owned UE SoundWave assets and wire `BoltCycle` / `LeverCycle` without overstating exact weapon identity.
- Keep `runtime_ready=false`, `ue_import_pending=true` and item 16 unchecked until UE import/wiring and local audibility/timing acceptance are factual.
