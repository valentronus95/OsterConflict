...
31. **Use component-first local UE debugging; the user is not the first test environment.** After a fail-closed local chain stops at one component, exhaust all source/static/preflight checks for that component before requesting another local UE run, including current UE API, frame/key/duration/envelope math, base/wrapper assertions, async compilation teardown, launcher return contracts and stale verifiers. Then rerun only the failed component until it passes on the current head; do not make the user repeat earlier phases that already passed. Run the full multi-phase evidence chain once only after affected components have individually passed, and run full gameplay only as consolidated acceptance rather than as a script debugger. For PASS45, `_DOCS/PASS45_COMPONENT_FIRST_UE_DEBUGGING_PROTOCOL.md` is binding.
