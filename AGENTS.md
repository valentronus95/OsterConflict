...
30. **Continue long-running TZ work from the latest factual checkpoint, not from zero, and do not stall on a deferred user-only gate.** When the user asks to continue a named TZ/pass from its latest factual/current checkpoint, first reconcile only the current branch, HEAD, active PR, recent commits, CI and the pass history/checkpoint. Consume newer parallel-chat commits, then resume the first factual open item and its direct dependencies. Do not replay already accepted/closed analysis. If that first-open item has exhausted all safe remote work and is blocked only by deferred user-local UE evidence, leave it unchecked, record the exact pending evidence, and continue the next remote-preparable item/direct dependency in the same consolidated acceptance batch. Do not stop every continuation at the same local-only blocker while useful remote work remains. A broad re-audit is allowed only when ownership/architecture, engine/license, merge history, direct runtime evidence, or checkpoint integrity materially invalidates the previous checkpoint. For `PASS45_RUNTIME_RECOVERY_TZ.md`, `_DOCS/PASS45_CHECKPOINT_CONTINUATION_PROTOCOL.md` is binding.
31. **Use broad batch-first user runtime acceptance and component-first corrective debugging; the user is not the first test environment.** Exhaust source/static/preflight/CI work before requesting local UE. While the user has deferred local execution, a per-item or per-weapon UE 5.8 acceptance seam is **not** a blocker to later remote-preparable PASS45 work. Do not ask the user to test each weapon, model, animation, SoundWave, vehicle or visual tweak after each small change. Continue as far as safely possible toward one broad integrated PASS45 package containing the intended weapon work, first-person hands/arms presentation, other production model replacements, tracked vehicle integration and remote-preparable graphics/visual improvements. Keep local-only items factually open and keep runtime/merge truth flags false; never fabricate acceptance. Ask for the planned local UE 5.8 check only when that broad integration batch is sufficiently prepared for one useful end-to-end visual/audio/gameplay session, or when a genuinely local-only fact blocks further safe remote work across the remaining batch. After that integrated run, collect one defect list, fix it as a batch, and rerun only failed components during corrective debugging before a final consolidated acceptance. For PASS45, `_DOCS/PASS45_CHECKPOINT_CONTINUATION_PROTOCOL.md` and `_DOCS/PASS45_COMPONENT_FIRST_UE_DEBUGGING_PROTOCOL.md` are binding.
32. **User-facing status and required actions must be unmistakable and written in plain language.** The user works on multiple projects and is not expected to decode long technical reports. Lead with ordinary Ukrainian: what was done, what remains, what is next, and official progress; technical verifier/workflow/SHA details are secondary. If no user action is required, state `ВІД ТЕБЕ ЗАРАЗ НІЧОГО НЕ ПОТРІБНО.` If a local check becomes a genuine hard blocker, state `ПОТРІБНА ТВОЯ ПЕРЕВІРКА.` prominently, explicitly name `Oster Conflict / PASS45`, explain why remote work cannot continue safely without it, give the smallest exact action, and say what result/evidence is needed. Never bury required user action in technical prose.
