...
31. **Use batch-first user runtime acceptance and component-first corrective debugging; the user is not the first test environment.** Exhaust source/static/preflight/CI work before requesting local UE. Do not ask the user to test each weapon after each small change. Prepare the intended weapon set as far as possible, then request one consolidated current-head UE 5.8 weapon visual/audio/gameplay acceptance session. If that consolidated run identifies a specific failed component, fix it remotely first and rerun only that failed component during corrective debugging; do not repeat already-passing phases. A pre-batch single-component user run is allowed only when a genuinely local-only fact is a hard blocker that prevents further safe remote work, or when the user explicitly requests it. For PASS45, `_DOCS/PASS45_COMPONENT_FIRST_UE_DEBUGGING_PROTOCOL.md` is binding.
